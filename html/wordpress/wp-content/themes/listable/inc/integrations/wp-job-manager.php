...
function listable_change_job_into_listing( $args ) {
	...
	$args['description'] = sprintf( esc_html__( '', 'listable' ), $plural );
	// This is where you can create and manage %s.
...
function custom_submit_job_form_fields( $fields ) {
	...
	$fields['job']['job_description']['description'] = sprintf( '<span class="description_tooltip right">%s</span>', '<a href="http://nabomhouse.com/listings/%EC%97%B0%EC%A3%BC%EB%84%A4-%ED%95%98%EC%88%99/">예시</a>' );
	...
	if ( $instance->get_job_id() ) {
		...
		<form method="post" id="job_preview" action="<?php echo esc_url( $action ); ?>">
			<div class="job_listing_preview_title">
				<input type="submit" name="continue" id="job_preview_submit_button" class="button job-manager-button-submit-listing" value="<?php echo apply_filters( 'submit_job_step_preview_submit_text', __( '완료', 'listable' ) ); ?>"/>
				<input type="submit" name="edit_job" class="button job-manager-button-edit-listing" value="<?php _e( 'Previeww', 'wp-job-manager' ); ?>"/>
