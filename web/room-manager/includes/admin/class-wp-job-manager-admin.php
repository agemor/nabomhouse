...
class WP_Job_Manager_Admin {
	...
	public function admin_menu() {
		add_submenu_page( 'edit.php?post_type=job_listing', __( '설정', 'wp-job-manager' ), __( '설정', 'wp-job-manager' ), 'manage_options', 'job-manager-settings', array( $this->settings_page, 'output' ) );

		if ( apply_filters( 'job_manager_show_addons_page', true ) )
			add_submenu_page(  'edit.php?post_type=job_listing', __( 'WP Job Manager Add-ons', 'wp-job-manager' ),  __( '추가기능', 'wp-job-manager' ) , 'manage_options', 'job-manager-addons', array( $this, 'addons_page' ) );
	}
